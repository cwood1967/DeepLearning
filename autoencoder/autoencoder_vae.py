...
def train(mmdict, df, params, ndisp, saveto=None):
    ...
    z_mean, z_logstd = network_vae.encoder(images, latent_size, droprate=droprate, is_train=True,
                          nfilters=enc_sizes, stdev=stdev, denoise = denoise)
    
    vae_z = network_vae.guess_z(z_mean, z_logstd, batchsize, latent_size)
    
    sdd = network_vae.decoder(vae_z, nchannels=nchannels, width=width,
                          droprate=droprate,
    ...
    opt = network_vae.model_opt(loss, learning_rate)
        ...
        if restore:
            ...
            for e in range(nepochs):
                ...
                while start < len(df) // batchsize - 1:
                    ...
                    asdd, aenc, az, aient, agent, _ = sess.run([sdd, vae_z, loss, ient, gent, opt],
                                                 feed_dict={images: batch})
                    ...
                    if ib % ndisp == 0:
                        sdh0r = asdd[ni]
                        test_he = sess.run(vae_z, feed_dict={images: test_batch})
                        test_sdd = sess.run(sdd, feed_dict={vae_z: test_he,
                                                            images: test_batch})
