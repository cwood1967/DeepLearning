...
def train(mmdict, df, params, ndisp, saveto=None):
    ...
    mix = network.mixture(enc_stack, nclusters)  ## returns tensor with nclusters probabilities
    loss, irploss, ixloss, ient, gent = network.mix_loss(images, sdd_stack, mix, 20, 800)
        ...
        if restore:
            ...
            for e in range(nepochs):
                ...
                while start < len(df) // batchsize - 1:
                    ...
                    asdd, aenc, amix, az, airploss, aixloss, aient, agent, _ = sess.run([sdd_stack, enc_stack, mix, loss,
                                                                      irploss, ixloss, ient, gent, opt],
                                                 feed_dict={images: batch})
                    ...
                    if ib % ndisp == 0:
                        ...
                        test_sdd, test_mix = sess.run([sdd_stack,mix], feed_dict={enc_stack: test_he, images: test_batch})
                            
                        print(test_he.shape)
                        print('Epoch: ', e, 'Iteration: ', ib, 'Loss: ', az, airploss, aixloss, aient, agent)
                        '''
